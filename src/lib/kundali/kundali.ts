...
export interface KundaliData {
  ...
  ascendant: number; // rashi index 0-11
  ascendantDegree: number;
  ascendantNakshatra: string;
  planets: PlanetPosition[];
  houses: number[][]; // planets in each house (by planet index)
  moonSign: number;
  sunSign: number;
  nakshatra: string; // birth nakshatra (Moon's)
  nakshatraPada: number;
  dashaLord: string;
}
...
// Approximate Ayanamsa (Lahiri) for a given Julian Day
function getAyanamsa(jd: number): number {
  const T = (jd - 2451545.0) / 36525.0;
  // Lahiri ayanamsa approximation
  return 23.856 + 0.0138 * (T + 0.5);
}

// Simplified planetary longitude calculations (tropical, then subtract ayanamsa)
...
// Approximate planet longitudes using mean elements
function calcPlanetLongitude(planet: Planet, jd: number): { longitude: number; retrograde: boolean } {
  const T = (jd - 2451545.0) / 36525.0;
  const d = jd - 2451545.0;

  // Mean orbital elements (simplified)
  const elements: Record<string, { L: number; rate: number; period: number }> = {
    Mars: { L: 355.45, rate: 0.5240208, period: 686.98 },
    Mercury: { L: 252.25, rate: 4.0923344, period: 87.97 },
    Jupiter: { L: 34.40, rate: 0.0830853, period: 4332.59 },
    Venus: { L: 181.98, rate: 1.6021302, period: 224.70 },
    Saturn: { L: 49.94, rate: 0.0334442, period: 10759.22 },
  };

  if (planet === "Rahu") {
    // Mean node of the Moon (Rahu)
    const lon = norm360(125.04452 - 1934.136261 * T);
    return { longitude: lon, retrograde: true }; // Rahu is always retrograde
  }

  if (planet === "Ketu") {
    const rahuLon = norm360(125.04452 - 1934.136261 * T);
    return { longitude: norm360(rahuLon + 180), retrograde: true };
  }

  const el = elements[planet as keyof typeof elements];
  if (!el) return { longitude: 0, retrograde: false };

  const meanLon = norm360(el.L + el.rate * d);

  // Simplified equation of center
  const e = 0.0167; // approximate eccentricity
  const M = meanLon * Math.PI / 180;
  const correction = 2 * e * Math.sin(M) + 1.25 * e * e * Math.sin(2 * M);
  const lon = norm360(meanLon + correction * 180 / Math.PI);

  // Simplified retrograde detection based on synodic period
  const sunLon = calcSunLongitude(jd);
  const elongation = norm360(lon - sunLon);
  // Superior planets are retrograde near opposition
  const isRetro = planet === "Mars" || planet === "Jupiter" || planet === "Saturn"
    ? (elongation > 150 && elongation < 210)
    : (elongation > 160 && elongation < 200); // inferior planets near inferior conjunction

  return { longitude: lon, retrograde: isRetro };
}

// Calculate Ascendant (Lagna)
function calcAscendant(jd: number, latitude: number, longitude: number, ayanamsa: number): number {
  const T = (jd - 2451545.0) / 36525.0;
  ...
  // Convert to sidereal
  asc = norm360(asc - ayanamsa);

  return asc;
}

function getLongitudeDetails(siderealLon: number) {
  ...
  return {
    rashi: rashi % 12,
    rashiName: RASHIS[rashi % 12],
    degree: Math.round(degree * 100) / 100,
    nakshatra: NAKSHATRAS[nakshatraIndex % 27],
    nakshatraLord: NAKSHATRA_LORDS[nakshatraIndex % 27],
    nakshatraPada,
  };
}
...
export function generateKundali(birth: BirthDetails): KundaliData {
  ...
  const decimalHour = hour + minute / 60 - birth.timezone; // convert to UT

  const jd = toJulianDay(year, month, day, decimalHour);
  const ayanamsa = getAyanamsa(jd);

  // Calculate ascendant
  const ascLon = calcAscendant(jd, birth.latitude, birth.longitude, ayanamsa);
  const ascDetails = getLongitudeDetails(ascLon);
  ...
  const planetPositions: PlanetPosition[] = PLANETS.map(planet => {
    ...
    if (planet === "Sun") {
      tropicalLon = calcSunLongitude(jd);
    } else if (planet === "Moon") {
      tropicalLon = calcMoonLongitude(jd);
    } else {
      const result = calcPlanetLongitude(planet, jd);
      tropicalLon = result.longitude;
      isRetrograde = result.retrograde;
    }

    const siderealLon = norm360(tropicalLon - ayanamsa);
    const details = getLongitudeDetails(siderealLon);
  ...
  // Build houses (Whole Sign houses from Ascendant)
  const ascRashi = ascDetails.rashi;
  const houses: number[][] = Array.from({ length: 12 }, () => []);

  planetPositions.forEach((pp, idx) => {
    const houseNum = ((pp.rashi - ascRashi + 12) % 12);
    houses[houseNum].push(idx);
  });
  ...
  return {
    birthDetails: birth,
    ascendant: ascRashi,
    ascendantDegree: ascDetails.degree,
    ascendantNakshatra: ascDetails.nakshatra,
    planets: planetPositions,
    houses,
    moonSign: moonPos.rashi,
    sunSign: sunPos.rashi,
    nakshatra: moonPos.nakshatra,
    nakshatraPada: moonPos.nakshatraPada,
    dashaLord: moonPos.nakshatraLord,
  };
}
...
export function calcVimshottariDasha(data: KundaliData): DashaPeriod[] {
  ...
  for (let i = 0; i < 9; i++) {
    ...
    for (let j = 0; j < 9; j++) {
      ...
      const adYears = totalYears * DASHA_YEARS[adPlanet] / 120;
...
export function getGemstoneRecommendations(data: KundaliData): GemstoneRecommendation[] {
  ...
  const ninthLord = RASHI_LORDS[(data.ascendant + 8) % 12];
